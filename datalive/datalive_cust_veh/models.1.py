...
class InsurancePolicyNumber(models.Model):
    ...
    vehicle_category = models.ForeignKey('VehicleCategory', blank=True, null=True, on_delete=models.SET_NULL)
...
class VehicleCategory(models.Model):
    """Model to represent a vehicle caetgory that can be used on may other models. 
    e.g. ODF category can be used on Vehicle, insurance policy number and FAQ's"""
...
class FAQ(models.Model):
    ...
    def __str__(self):
        return '%s | %s | %s' % (self.title, self.vehicle_category, self.customer.name)
...
class Livery(models.Model):
    """Model to represent schematic views of a vehicle. """
...
class LiveryView(models.Model):
    ...
    name = models.CharField(max_length=255)
    view_name = models.CharField(max_length=10, choices=LIVERY_VIEWS, null=True)
...
class Vehicle(models.Model):
    ...
    TODO:
        * Move message_status to a seperate model
    """
    VEHICLE_TYPES = (
        ('BIK', 'Bike'),
        ('CAR', 'Car'),
    ...
    """reference to the schematic views for vehicle."""
    ...
    type = models.CharField(max_length=3, choices=VEHICLE_TYPES, default='VAN')
    """Type of vehicle."""
    # vehicle_category = models.CharField(max_length=5, choices=VEHICLE_CATEGORIES, default='FLEET')
    vehicle_category = models.ForeignKey('VehicleCategory', blank=True, null=True, on_delete=models.SET_NULL)
    """A category label a customer can assign to a vehicle"""
    ...
    #"""This is only used for some vehicles with the messaging system installed, may be best to move out to a seperate model"""
    ...
    # rear_tyre_size = models.CharField(max_length=15,null=True, blank=True)
...
class VehicleGroupContact(models.Model):
    ...
    email = models.EmailField(max_length=256, blank=True, null=False)
    web_url = models.URLField(max_length=256, blank=True, null=False)
...
class VehicleGroup(models.Model):
    ...
    @staticmethod
    def get_all():
        return VehicleGroup.objects.all().select_related('customer').prefetch_related('vehicles').defer('vehicles__trackers', 'vehicles__customer')
